...
$rowCount = mysqli_num_rows($queryResult);
 ...
 echo "<table border='1' cellpadding='10'>";
		 ...
		 if($rowCount > 0)
		 {
			 ...
			 {	$query1 = "select * from product_master where product_id = '$row[product_id]'";
				$queryResult1 = $conn->query($query1);
				$row1 = mysqli_fetch_array($queryResult1);
				echo "<tr>";
				echo "<td> $row[product_id]</td>";
				echo "<td> $row1[product_name]</td>";
				echo "<td> <a href=\"product_details_add.php?name=$name&id=$row[product_id]\">Add</a></td>";
				echo "<td> <a href=\"product_details_edit.php?name=$name&id=$row[product_id]\">Edit</a></td>";
				echo "<td> <a href=\"product_details_view.php?name=$name&id=$row[product_id]\">View</a></td>";
				echo "</tr>";
			 }
		 }
		 else
...
for($i=1;$i<=$total_pages;$i++)
{
	if($i==$pno)
	{
		$pagelink.="<a class='active' href=\"product_details_list.php?page=$i&name=$name\">$i</a>";
		$str=" ";
		$pagelink.=$str;
	}
	else
	{
		$pagelink.="<a href=\"product_details_list.php?page=$i&name=$name\">$i</a>";
		$str=" ";
		$pagelink.=$str;
	}
}
